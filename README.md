...
Built a recall-first churn model with XGBoost (benchmarked against Random Forest), with the operating threshold frozen on validation via bootstrap; no test-set tuning. Deployed a 0.575 policy that delivers ~52% recall at ~34% precision on test, AP 0.354 (~2x baseline 0.175), 26.7% flag rate; plus validation-derived risk tiers, a model card, confusion matrix, and F-score feature importance to guide retention strategy.
